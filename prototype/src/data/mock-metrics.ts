...
export const mockWorkflowTrends: WorkflowTrend[] = [
  ...
  },
  {
    ...
    alertRatio7Day: [0.40, 0.42, 0.43, 0.44, 0.43, 0.45, 0.46],
  },
]
...
export const mockKriCatalogue: KriCatalogueEntry[] = [
  {
    id: 'R1',
    name: 'AI 入力承認率 推移悪化',
    triggerCondition: '週次平均が 目標仮説 (≥ 99%) を 2 週連続 下回り',
    responseAction: '手順管理者 へ通知、AI 管理者 が確認',
    state: 'normal',
    status: '仮説 / 要検証',
  },
  {
    id: 'R2',
    name: '人手上書き率 急増',
    triggerCondition: '日次平均が 目標仮説 (≤ 1%) を 3 倍以上 超過',
    responseAction: '手順管理者 へ通知 + 段階の強制引き下げ を検討',
    state: 'normal',
    status: '仮説 / 要検証',
  },
  {
    id: 'R3',
    name: 'Alert 誤検知 急増',
    triggerCondition: 'Alert 精度 が週次で 50% 下回り',
    responseAction: '承認方針の Alert 条件 見直しを起動',
    state: 'caution',
    status: '仮説 / 要検証',
  },
  {
    id: 'R4',
    name: '承認者差戻し率 急上昇',
    triggerCondition: '週次平均が 目標仮説 (≤ 1%) を 5 倍以上 超過',
    responseAction: '業務責任者 へ通知 + 段階の強制引き下げ を検討',
    state: 'normal',
    status: '仮説 / 要検証',
  },
  {
    id: 'R5',
    name: 'UI 変更 検知',
    triggerCondition: '業務システム画面の 表示構成 変更による操作失敗',
    responseAction: 'AI 管理者 へ通知 + Agent 指示書 を確認',
    state: 'normal',
    status: '仮説 / 要検証',
  },
  {
  ...
  },
  {
    ...
    responseAction: 'AI 管理者 へ通知、Agent 版数 を強制更新',
    state: 'normal',
    status: '仮説 / 要検証',
  },
  {
    ...
    responseAction: '入力 PDF 受領元との 連絡経路 見直しを起動',
    state: 'normal',
    status: '仮説 / 要検証',
  },
]
