...
- 承認者画面は実装せず、`BusinessApprovalChip` + 静的 mock (`demo/static-mocks/business-approval-view.html`、Day 20 実装済) で代替
...
| BusinessApproval chip | Primary + Diff borrow | sticky bottom action bar の右端、UI 表示文言は **`承認者承認: 承認待ち`** (v1.4 Commit 4 U-8 で旧 `業務承認:` から rename、component 名 `BusinessApprovalChip` は internal、UI 表示には出さない) |
...
| ----------------------- | -------------------------- |
| `BusinessApprovalChip` | `承認者承認: 承認待ち` (v1.4 Commit 4 U-8、旧 `業務承認:` から rename) |
...
`BusinessApprovalView` は `demo/static-mocks/business-approval-view.html` (Day 20 実体化済) として静的 mock のみ、React route 対象外。CaseReview footer の `BusinessApprovalChip` click で別タブで開く (Day 14 medium-fi で disabled wrapper title 経由 → Day 20 で再 enabled、`window.open(import.meta.env.BASE_URL + 'demo/business-approval-view.html')` 動作)。
...
7. **遷移**: 差戻し click → `/cases/:id/comment`、承認 click → state 変更 + Inbox queue 戻る、`BusinessApprovalChip` click → 静的 mock 別タブ (Day 14 medium-fi で disabled wrapper title 経由 → Day 20 で再 enabled、`window.open(import.meta.env.BASE_URL + 'demo/business-approval-view.html')` 動作)
...
9. **Day 12 Page 7 実装注記** (commit `4ffd337` + CR R46 patch): event model (15 行、関連項目 含む実 18) は `mock-audit.ts` `AuditEvent` interface (camelCase TS + JSDoc snake_case SSOT mapping) で type 定義、DOC-KNW-04 §8.1 と整合。**Layout** (Operational Premium Light + 他 7 page register 共通): Sticky PageHeader (breadcrumb 2-level + h1 + 「直近 30 日 (検証用)」chip + meta「15 項目構造 · 関連項目 含む実 18」+ workflow filter 3 chip [全業務 / UC-BO-01 / UC-BO-02、useState、aria-pressed、page-global scope]) + SSOT framing 注 (slate-50 inset + History icon、「監査イベントは 15 項目構造 で記録」/ 行を選択すると 15 項目 詳細展開) + 監査イベント 時系列 (timestamp DESC sort、各 row icon + EVENT_TYPE_LABEL + case_id mono + workflow_id mono + 過去案件への影響 chip if rule_update_alert + timestamp + actorLabel mono + summary、`useState expandedId` で 1 row at a time toggle) + 展開時 15 項目 詳細 (**CR R46 paradigm = JP label primary + SSOT snake_case key sub-caption + JP value**、`DetailPanel` + `DetailRow` 内部 component、案件 ID / 業務 / Agent / Prompt 設定 版数 / Tool 設定 版数 / Model 設定 版数 / 入力 PDF ハッシュ / AI 提案 ID / 人手判断 ID / 差戻し分類 / 承認済ナレッジ 参照 ID (wide row) / 承認時刻 + 承認者 / 反映差分 ID / ロールバック参照 / 操作画面記録 ID、note で「次の実装段階」「検証用項目」明示、R46 paradigm で `skeleton` / `scope-out` / `navigation` / `screenshot` も localize) + Sticky footer (ダッシュボード戻り link + 検証用 監査機能の拡張は次の実装段階で対応 caption)。**EVENT_TYPE_LABEL** (10 種): system_intake → PDF 受付 / ai_input → AI 入力 / human_review → 入力者確認 / human_sendback → 入力者差戻し / ai_analysis → AI 日次分析 / proposal_approve → 手順承認 / business_approve → 承認者承認 / reflect → 反映 / rule_update_alert → 関連ルール更新 / config_approve → 設定承認。**EVENT_TYPE_STYLE**: icon + badge state-conditional (system/ai = primary, human review = emerald, sendback/rule_update = amber, approve/reflect = emerald)。**適用範囲 2 関連ルール更新 Alert** (DOC-UI-03 §6.2): `rule_update_alert` event type で実装、timeline 上に inline 表示 (banner ではない、row 配下 nested ではなく row として timestamp 順 inline)、row 全体に amber-50/30 tint + 「過去案件への影響」amber chip (CR R46 M2 で 「適用範囲 2」 単独表示から semantic 補強)、文言「YYYY-MM-DD に [手順名] v0.X が承認されました (本案件の処理時の版は当時のまま 監査証跡 に保持)」。**mock-audit.ts** (13 events): CASE-2026-0142 (Demo Chapter 1、2 events) + CASE-2026-0118 (Demo Chapter 2 narrative 中核、9 events incl. 過去案件への影響 Alert) + CASE-2026-0148 (UC-BO-02 Dashboard 要注意、2 events)、Demo Chapter 2 全 flow trace (PDF 受付 → AI 入力 → 入力者差戻し → AI 日次分析 → 手順承認 → 関連ルール更新 Alert → 入力者再確認 → 承認者承認 → 反映)。**CR R44 governance term paraphrase 規範 pre-flight 適用** + **CR R46 二次 hygiene**: review / trigger / Forced Downgrade / upgrade / precision / layout (R44) + schema / event row / click / navigation / skeleton / scope-out / screenshot / 本 case (R46) は mock data summary / actorLabel / EVENT_TYPE_LABEL / UI copy / DetailRow note から完全排除、internal SSOT 参照 (snake_case schema field 名) は **JSDoc + DetailRow schemaKey sub-caption** として keep (R46 paradigm: JP label primary + snake_case key sub-caption の責務分離)。**国際送金** (restricted boundary pack) は `workflow_id` 含めず、`mock-audit.ts` 対象外 (DOC-OV-00 §2.1)
...
- **表示内容**: 「承認者承認: 承認待ち / 承認済み」+ 状態 badge (v1.4 Commit 4 U-8、旧 `業務承認:` から rename)
- **click action**: `window.open(import.meta.env.BASE_URL + 'demo/business-approval-view.html', '_blank', 'noopener,noreferrer')` で別タブ起動 (Day 20 実体化済、`vite.config.ts` `base: process.env.VITE_BASE_PATH ?? '/'` と整合、dev / root deploy / GitHub Pages sub-path deploy 全て resolves)。静的 mock の canonical = `demo/static-mocks/business-approval-view.html`、Vite が `prototype/public/demo/business-approval-view.html` (symlink) を serve
- 本 prototype 内では実承認 flow は実装しない (mock state 切替のみ)
- a11y: button role、aria-label「承認者承認画面を別タブで開く (Session 4 slide 用 static mock)」、title 同文。Day 20 で onClick 設定 (AST no-op gate pass)。Day 14-18 medium/high-fi 期間は disabled wrapper title 経由 (Day 20 で再 enabled に巻き戻し)
...
| `medium` | amber dot (`bg-amber-500`) + 「確認済 (未承認)」      | reviewed staging。引用根拠 対象外、low と同等扱い                     |
| `high`   | emerald dot (`bg-emerald-600`) + 「承認済」            | compiled approved。引用根拠として使用可                               |
