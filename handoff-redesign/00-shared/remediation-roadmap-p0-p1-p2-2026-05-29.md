...
>
> **CR 収束 (2026-05-30)**: 計画構造は ~6 CR round + external CR で converged。**本 statement 以降、plan 構造の追加 CR を禁止**。次アクションは W0 gate 着手。新規 cross-cutting 要件が出た場合のみ §1b/§4 に追記し、構造 re-CR はしない。
...
> **本 roadmap は未着手の forward-looking 実装計画である**。実装コードが各 wave の記述どおりに変更されていないこと (例: `persist.ts` が `SCHEMA_VERSION=2` のまま、`proposal/sendback`/`session/switchActor` action 不在、口座開設 case 不在、route 数 9) は **正常な現状であり defect ではない**。各項目は指定 wave で実装する。批判レビューは「コードが計画どおり実装済か」ではなく「**計画 (設計・順序・gate) が妥当か**」を評価すること (round-2 CR が前者を誤って評価したため明記)。
...
| **W0 — functional foundation** | 全画面が継承 | contrast SSOT (`--color-fg-tertiary #475569`) / tone SSOT (却下=inset + trustTone/toastTone) / strict-ts Stage1 (app 39→0) / icon-suffix gate。**visual token (shadow/type/density/motion) は定義のみ、適用は W1 (非 gating、functional-first)** | **P1-4, P1-9(Stage1)** |
| **W1 — 既存 9 画面 per-screen refine** | 既存 9 画面 | P0-done 保全 + per-screen findings 解消 + W0-visual 適用 + **strict Stage2 (W1末: test 107→0 + check:types:test)** | **P1-1, P1-5, P1-6(行), P1-8** |
| **W2 — 新 3 画面 + IA + store** (W2a→b→c) | 9→12 画面 | **W2a** store/schema (`agent/approvePromotion`・`sendbackPromotion`・`case/escalate` + `readNotificationIds` + `promotionStatus`/`escalation` 加法 + **SCHEMA 単一 4→5 bump** + promotionRequested→promotionStatus 移行 8ref/6file) → **W2b** /search + /inbox + ProcessSelector dropdown a11y → **W2c** /oversight + Observatory drill/CrossLedger/Period + IA sync (typology A×2/B×7/C×3=12 byte-identical) | **P1-2, P1-3, P1-6(dropdown), P1-7** |
| **W3 — production-ready completion** | 12 画面 | P2 batched backlog (§4) + §8 残存 gap。**§4 必須昇格群** (下記 A-5) を含む | **P2 G1-G11** |

> **W0 着手条件**: master plan rebaseline 承認は **不要** (W0 は画面追加なし、§5 の rebaseline 必須は画面追加の W2 = P1-2/P1-3)。**W2/W3 着手前に master plan rebaseline 承認が前提** (§5)。route 名は `/inbox` に統一 (`/notifications` 系記述を一掃)。business-approver = unified `/oversight` 1-route。
>
> **W3 必須昇格群** (P2 batched から W3 mandatory へ格上げ、§4 で個別 mark): **reversal** (C3 訂正/取消) / **manual entry** (C4 手動起票) / **modal hardening** (inert/scroll-lock/unsaved-guard、§4.2 G5) / **multi-tab** (storage event、§8) / **axe** (12画面 axe smoke、§4.2 G9/G11) / **@media print** (§8)。これらは production-ready の必須要件であり optional backlog 扱いにしない。
...
| **P1-W7** | P1-3 (業務責任者 surface、11→12 画面) ／ P1-7 (Observatory drill + 案件横断台帳) ／ P1-8 (before/after) | check:all green / P2B-4 gate / demo-script 2 journey / typology lock 12 画面 |
...
> finding 軸 (§1.1 / §3 / §4) を補完する **screen 軸の完成定義**。各画面の「何をもって完成とするか」を type × 主 refine × 解消 finding で定義。W1 (既存 9 画面 refine) + W2 (新 3 画面) を 1 表で閉じる。実装詳細は §3 deep-plan を per-component pointer として参照。P0-done (B1/B2/B3/B4/A2/A3/A4/A5/I2/DC1/RD1/C6/R4) は **保全** (各 C 型画面で regression させない)。
...
|---|---|---|---|---|
| **/search** | B | SearchResultList + SearchInput、種別 chip + mono ID、row→detail (`?q=` back)、zero-result 専用文言 | §7 #9 (search), §6 novelty=new | §3.2 (P1-2) |
| **/inbox** | B | NotificationRow + NotificationList、bell 未読 live、role-filtered (currentActor 宛) | §7 #9 (notify), §5a 欠落 | §3.2 (P1-2) |
| **/oversight** | C (内部 segment) | DecisionQueueTable + SegmentTabs (role=tablist)、設定承認 + 裁定、row→既存 owner-mode C-detail drill。SoD = P0 hard-block helper 再利用 | §7 #10, §5a, §8 persona gap | §3.3 (P1-3) |
...
| `TopBar.tsx:19-27,32-38` | 検索 silhouette → 機能 input (value/onChange、Enter で /search navigate)。BellIcon → NavLink /notifications + 赤ドットを store 派生未読数連動 (>0 のみ) |
...
| `App.tsx` / `Sidebar.tsx` | /search + /notifications を AppShell 配下に追加 (9→11 画面)。Sidebar nav 追加は judgement (最小は TopBar のみ) |
...
| `Sidebar.tsx:32-52` / `App.tsx:27-39` | ─承認─ group (/config-approvals, /escalations) + /business-approver。9→12 画面 (typology lock 更新) |
...
- **判断 gate**:
  - **JG-1 (IA scope)**: (a) landing + 2 queue の 3 画面 / (b) 統合 queue 1 画面 (tab 分割) / (c) mode 配線のみ landing/queue なし / (d) **scope-0** (mode 間借りのまま R1 へ carve)。trade-off: (a) IA 網羅最大だが 9→12 で polish target 拡大 (95% equal と衝突) / (b) 中庸 / (c) §8 persona IA 欠落が残り再指摘 / (d) frame C と矛盾。**推奨 (b)** 単一統合 queue + landing は role landing 代替。**frame C ゆえ (a) も妥当 → user 判断。選択後に polish tier (95% equal / prototype-tier 低-fi) を pair 確定し effort=L を再確定**。
  ...
  - **JG-4 (typology lock 更新の可否)**: 9→12 を許容するか。frame C 許容前提だが master plan rebaseline 承認 (P0-W0) が前提。新 3 画面の polish tier は user 判断。
...
| file:line | 変更 |
|---|---|
| `index.css:18` | fg-subtle は**削除も値変更もしない** (disabled/icon の意図的弱コントラストを保つ)。意味テキスト用 AA-pass token `--color-fg-tertiary-text: #475569` (全背景 7.58/7.24/6.92:1) を追加。コメントで用途分離明記 |
| `Cases/Agents/Observatory/ProposalDetail/MetricVsThreshold/ReconcilePanel/FieldActionModal/Sidebar/LifecycleStepper/DataTable` (上記 ~20 箇所) | text-[fg-subtle] → fg-tertiary-text。disabled/icon 箇所は据え置き。Sidebar:123 は **P0 Gate4 の useCurrentActor 化後に当てる** (二重編集回避) |
| Observatory panel-inset 上 fg-muted 行 | 全数監査し panel-inset 上は fg-tertiary-text/fg へ昇格 |
...
- **判断 gate**:
  - **代替 token 選択**: (a) **fg-tertiary-text=#475569** (tertiary 階層維持) / (b) fg-muted へ集約 (階層廃止、panel-inset は別途昇格要) / (c) fg-subtle 値を上げる (disabled/icon の弱コントラスト意図を破壊)。**推奨 (a)**。視覚 hierarchy は user 決定。
...
- **reconcile**: audit §3.5 SC1 + AR2 を 1:1 解消 (#14)。9 画面 screen-level axe (P2) とは別レイヤ (補完)。canonical-design-spec §8 keyboard 行追記は AR1 (P1-4) の SSOT 明文化と pair。
...
| `ReconcilePanel.tsx:62-71,118-119` | (a) 要確認カードに previousValue 有る field の「現在の登録値 → 確定値」行追加 (表示値 = `humanValue ?? aiValue` で P0 B1 と整合) (b) 確認済行 :119 に inline 併記。矢印は lucide ArrowRightIcon + fg-tertiary-text (P1-4 token)。**P0 B1 と同一行 :119 を編集するため統合実装必須** |
...
- **検証**: check:all green (button 新設なしで自動 pass) / detail-routing.test (0142 新住所が previousValue 持つ / 口座開設は持たない) / negative grep (mock-case-detail で change.from 参照 0→1+ / ReconcilePanel に旧値 hardcode 0) / design grep (off-token hex 0 / ArrowRightIcon / fg-tertiary-text) / screen evidence (0142 で現行登録値→変更後、P0 B1 override 後に右辺更新、口座開設は非表示)。
...
- **検証**: 実測再現 (strict+NUIA で **app 39 error → guard 後 0**、Stage1) / `npm run build` clean build 0 error / check:types:test (**test config 107→0**、Stage2) / check:all green / negative grep (`as any` / 不要 `!` 新規混入 0) / legacy 不変 (strict-exempt) / probe artifact cleanup。
- **dependsOnP1**: なし。**effort**: AI=M / review=S / judgement=med。
...
> | **manual entry** (C4 手動起票) | audit §7 P2 / C4 / 旧 overlay W3b。Cases に「新規案件作成」+ 全項目手入力 form (AI 障害時の業務継続)。route 化 (typology 13) vs modal (12 維持) は着手時 decision | draft 作成 test + (route 時) typology 再同期 |
...
> | **axe** (12 画面 axe smoke) | §4.2 **G9/G11** (現状 2 component → 12 route 網羅) | axe 0 violation × 12 route |
...
| **G9** | build-health (strict/NUIA/test typecheck/9画面 axe/code-split) (5) | tsconfig 強化 + check:types:test + 9 route axe + React.lazy split | check:all に全 gate green | **P1-9 と統合済**、NUIA は P0 後 | M |
...
| **G11** | 9 画面 axe smoke + route-level code-split (2) | a11y smoke を 9 route 網羅 + React.lazy | axe 0 + chunk 分割 | P0 + 全 P2 design-a11y 後 (最後) | M |
...
|---|---|
| **9 画面 lock** (ia-overview-v2 / screen-contracts / coverage-matrix) | **P1-2 で 9→11 (検索/通知)、P1-3 で 11→12 (業務責任者 landing + 2 queue)**。typology lock を A×3/B×6/C×3 へ更新 |
...
- **Sprint H = P1-W7** (業務責任者 + Observatory + before-after、11→12 画面、judgement-heavy)
...
| **6/12 後送り (推奨)** | **P1-W6** (検索/通知、9→11)、**P1-W7** (業務責任者/Observatory drill/before-after、11→12) | 画面追加を伴い master plan rebaseline + polish tier 確定 + 契約 3 doc 更新が前提。judgement-heavy で 6/12 前の確定が困難。demo は P0 の 9 画面で中核 message を伝えられる |
