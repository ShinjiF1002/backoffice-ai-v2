...
**日付**: 2026-05-29 ／ **由来**: `user-perspective-ui-audit-2026-05-29.md` §7 (raw 121 → confirmed 112 = major 33 + minor 79、refuted 12。major 33 を P0 8 + P1 9 の work-item に consolidate、minor 79 を P2 backlog 29 batch 化) ／ **frame**: C (production-ready lens、即フル着手) ／ **新 baseline**: 本 roadmap が以降の実装 baseline。
...
frame C で P0 + P1 + P2 を 8 wave に直列/部分並行配置する。wave 順序は store hot-file 共有 (types/reducer/persist/seed/hooks) と Observatory.tsx の編集衝突を最小化する linearization。
...
| **P0-W1** | P0 B1 store / B4 SoD-actor / sendback-guard / SCHEMA_VERSION 2→3 集約 bump ＋ **[積層] P1-3 の store 拡張 (agent/approvePromotion・sendbackPromotion・case/escalate + AgentEntity/CaseEntity field) を同 bump に相乗り** ＋ [条件] P1-9 NUIA フラグ前倒し ON | check:all green / store.test 新署名 green / exhaustive never 全 case 網羅 (新 9 action) |
...
| **P1-W4** | P1-9 (strict + NUIA + test typecheck) ／ P1-4 (contrast SSOT + R7 gate) | check:all green (24→0 error) / fg-subtle negative grep 0 / R7 green |
...
**根本原因**: `tsconfig.app.json:23-27` が strict 系を一切設定せず linting 系のみ。empirical probe で確定: (1) strict 単独 = active 0 error / test 0 error (追加修正 ZERO)。(2) NUIA 単独 = active 24 error (store/hooks.ts 15 / pages 5 / Modal 2 / data 2)。pattern: dict map `s.caseOrder.map(id=>s.cases[id])` が `(T|undefined)[]` / array-index `STATUSES[i%len]`。reducer.ts は既に NUIA-clean (早期 return guard が canonical)。(3) tsconfig.test.json が `tsc -b` build graph 外で test 型未検査 (test-only NUIA error 37)。
...
- **検証**: probe 再現 (strict+NUIA で active 24 error → guard 後 0) / `npm run build` clean build 0 error / check:types:test (test+active 61→0) / check:all green / negative grep (`as any` / 不要 `!` 新規混入 0) / legacy 不変 (strict-exempt) / probe artifact cleanup。
- **dependsOnP1**: なし。**effort**: AI=M / review=S / judgement=med。
...
> **coverage-reconcile (最終 batch)**: audit §4 minor 79 件 ↔ 上記 batch itemTitle の cross-walk 表を張り、全 79 件がいずれかの batch に hit することを count gate で確認。意図的 defer は scope-out 明記。**未配置リスク minor を明示配置**: (1) sent-back→ready 遷移未実装 (reducer.ts:60-61、backend step ゆえ R0 では disclaimer or scope-out) → nav-content-queue / (2) sidebar user-area 非インタラクティブ div (Sidebar:117-125、P0 persona switcher と co-locate) → P0 Gate4 / (3) ヘルプ/用語集導線 不在 (legacy 隔離) → nav-content-queue / (4) @media print → design-a11y。
...
| P1-W4 | strict+NUIA 24→0 / fg-subtle negative grep 0 / R7 green | 4 画面で tertiary 視認 + hierarchy 維持 |
