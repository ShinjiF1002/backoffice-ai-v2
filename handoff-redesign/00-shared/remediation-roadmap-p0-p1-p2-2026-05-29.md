...
> **本 roadmap は一部実装済 + 残り forward-looking の実装計画である** (2026-05-30 更新)。**実装済**: P0 (SCHEMA_VERSION=4、`proposal/sendback`・`session/switchActor` action、口座開設 case、route 9) + W0 + W1-A = **P1-W5** (= P1-W4 strict Stage1+Stage2/icon/contrast/tone + P1-1 ProcessSelector→ViewContext + P1-5 Loading/Error CORE + P1-5-remainder detail not-found→EmptyState + permission-empty dead branch 除去)。**実装済 (追記 2026-05-30)**: **W2 全 wave (W2a/W2b/W2c)** = 9→14 画面 + 全 enhancement (検索/通知/keyboard a11y/業務責任者 3 画面/before-after/Observatory drill+横断台帳)、`remediation/w2-screens` (下記 W2*/P1-* 完了 note 群が as-built SSOT)。**未実装 (forward-looking)**: **W3 (P2) のみ**。未実装 wave のコードが記述どおりでないのは **正常であり defect ではない**。批判レビューは「未実装 wave の計画 (設計・順序・gate) が妥当か」+「実装済 wave の現物が gate green か」を評価すること (round-2 CR が前者を誤評価したため明記)。
>
...
>
> **★ W3 (production polish) 完遂 (2026-05-30)**: §4.0 必須昇格群 + §4.3 G2 + SLA + future-date を実装。`remediation/w3-polish` (#17 squash の origin/main から分岐)、8 commit (W3-S/a/b/c/e/f/g + review-fix)、check:all green (**test 212** / 21 files / build) + 本番 boot smoke。
> - **W3-S store+SLA core**: SCHEMA **5→6** (単一 bump、deploy は 6/12 demo と別日 = risk #1)。case/reverse (反映済の訂正/取消、不可逆 guard) + case/create (手動起票 draft、id 冪等) + store/hydrate を action union へ。CaseEntity.elapsedLabel(静的文字列)→receivedAt + `lib/dates` (NOW_ISO 固定 + elapsedLabelFrom/caseElapsedLabel、reflected→処理済)。
> - **W3a reversal UI**: CaseDetail reflected footer に訂正/取消 (useCanReverse、承認者/業務責任者のみ、入力者不可) + ReasonDialog + reversal banner。
> - **W3b manual-entry**: `/cases/new` route (**typology 14→15、A×3/B×9/C×3**)。CaseDraft form (業務 select + 全項目手入力 + 自動採番) + buildManualCaseDetail で store-only draft が CaseDetail で開ける。
> - **W3c Observatory §4.3 G2**: 横断台帳 actor/action FilterChip + pagination(pageSize 12) + 全 filter で page-reset。**§4 G7 future-date**: OBS lifecycle/ledger + Hub の 2026-05-31→2026-05-30 + check-design gate#4 (regression-sentinel)。
> - **W3e modal G5 + multi-tab**: Modal dirty-guard (破棄確認 overlay + 背景 inert + focus 移動) + body scroll-lock。StoreProvider window 'storage' listener + store/hydrate + savePersisted 同値skip (ping-pong 防止)。
> - **W3f @media print** (index.css のみ、shell chrome を隠し証跡を紙へ) / **W3g 15-route axe smoke** (構造 a11y、jsdom は contrast 非評価)。axe が顕在化した実 a11y bug 3 件を是正: MetricVsThreshold h3→h2 / DocumentViewer dl→div / ReconcilePanel card role=button 廃止。
> - **adversarial review remediation (5-dim Opus + per-finding verify)**: green gate を通過した実 bug 8 件を検出・是正 — reversal **訂正・取消とも sent-back** (ready 直行の false-success 廃止 + 再反映 trap 解消) / 取消 通知を 'reversal' kind で区別 (理由保持) / Modal 破棄 overlay の inert+focus / Observatory 0142 時系列を receivedAt 16:40 起点に rebase / future-date comment honesty / axe coverage (open modal + 非default tab) / ReconcilePanel aria-pressed。
> - **contract sync**: allowed-actions §1 (reflected 可逆、訂正/取消→sent-back) + screen-contracts §4 + canonical-design-spec §5 icon + typology 15 (6 recall surface + master plan) + axe gate 14→15。
> - **closure**: §4末 3 軸 count gate (major 33 ↔ §1 / minor 79 ↔ §4 / screen ↔ §1b) を機械確認、意図的 defer を scope-out 明記。**残アクション: 無し (W3 完遂、PR 提示可能)**。
...
| **W3 — production-ready completion** | 14→15 画面 (manual-entry route) | P2 batched backlog (§4) + §8 残存 gap。**§4 必須昇格群** (下記 A-5) を含む。manual-entry を route 化 → typology A×3/B×9/C×3=15 | **P2 G1-G11** |
...
>
> **W3 必須昇格群** (P2 batched から W3 mandatory へ格上げ、§4 で個別 mark): **reversal** (C3 訂正/取消) / **manual entry** (C4 手動起票) / **modal hardening** (inert/scroll-lock/unsaved-guard、§4.2 G5) / **multi-tab** (storage event、§8) / **axe** (15画面 axe smoke、§4.2 G9/G11、manual-entry route 追加で 14→15) / **@media print** (§8)。これらは production-ready の必須要件であり optional backlog 扱いにしない。
...
| `Observatory.tsx:130,199-211,247` | (a) case ID span を /cases/{id} Link、台帳行に rowHref、未達 KPI 行に /agents/{id} drill (b) 監査 ledger を DataTable (案件選択 + workflow + 期間/actor filter + pageSize + free-text 検索) に置換 (c) P0 Gate5(ii) lineage view と座席整合。**【as-built W2c-2/P1-7b: (b) は DataTable 全置換ではなく既存 table 構造を維持し workflow filter chip + free-text 検索 (Observatory ローカル、JG-2=b) + 「直近30日」固定ラベル (JG-4=b) を載せた。caseId 列で row drill。actor/action FilterChip + pageSize/pagination + 期間 selector は未実装 = W3/P2 へ明示 defer (時系列は固定 fixture で data shape 不足、JG-4=b 既定)。lifecycle のみ対象 case ラベル表示 (ledger は横断ゆえ非表示)】** |
...
- **検証**: check:all green / negative grep (OBS_CASE_ID 直書き 0 / 台帳行に rowHref / 未達 KPI に agentHref) / off-token hex 0 + JP-only (英語 schema 値は監査 raw ledger sanctioned 例外、見出しは P2 G8/G9 で和訳) / Playwright (台帳 row→口座開設 detail、未達 KPI→Agent detail、案件 selector(workflow filter)/検索/drill 動作。**pagination/actor-action 動作は as-built では未実装 = W3/P2 defer**) / lineage view と台帳の chrome 両立 / __tests__ drill/検索/workflow filter (observatory-drill.test as-built)。
...
> | **axe** (15 画面 axe smoke) | §4.2 **G9/G11** (現状 2 component → 15 route 網羅、manual-entry で 14→15) | axe 0 violation × 15 route |
...
| **G9** | build-health (strict/NUIA/test typecheck/15画面 axe/code-split) (5) | tsconfig 強化 + check:types:test + 15 route axe + React.lazy split | check:all に全 gate green | **P1-9 と統合済**、NUIA は P0 後 | M |
...
| **G11** | 15 画面 axe smoke + route-level code-split (2) | a11y smoke を 15 route 網羅 + React.lazy (code-split は defer) | axe 0 (構造、jsdom は contrast 非評価) | P0 + 全 P2 design-a11y 後 (最後) | M |
...
> **★ W3-END CLOSURE 実行結果 (2026-05-30、3 軸 count gate)**:
> - **major 33 軸**: P0 8 + P1 9 work-item は全て P0/W1/W2 で close 済 (store action 21 = §1 itemKey 全 hit、機械確認 `grep -cE "^  \\| \\{ type:" types.ts` = 21)。W3 は major を新規 close せず、regression 無し (check:all test 212 green) を再確認。
> - **minor 79 軸 — W3 で close**: reversal (C3) / manual-entry (C4) / modal-hardening (§4.2 G5) / multi-tab (§8) / **15画面 axe** (§4.2 G9・§4.3 G11) / @media print (§8) / **§4.3 G2** (Observatory actor/action filter + pagination) / **§4 G7** (future-date 統一、active-src 2026-05-31 grep 0) / SLA real computation (audit §3.2 scope-0 本実装)。
> - **screen 軸 (§1b)**: 15 画面 (A×3/B×9/C×3)、routes.test / routes-axe で全 render-gate + 構造 axe 0。新規 affordance (CaseDetail reversal、Cases→/cases/new) を §1b に追記済。
> - **意図的 scope-out (justification 付き、R1/backend へ defer)**:
>   - **sent-back→ready 遷移** (reducer): backend AI 再処理 step。prototype は disclaimer で表現、実装は Phase 1。reversal も sent-back 止まり (再処理は backend)。
>   - **manual-entry 自動採番の multi-tab 衝突**: count ベース id は 2 タブ同時 create で衝突し得る (minor)。実系は server 採番で解決、prototype は許容 (W3 adversarial review confirmed、defer)。
>   - **route-level code-split / React.lazy** (§4.2 G9・§4.3 G11 後半): perf optional、~459KB chunk は demo 非課題ゆえ defer。
>   - **Observatory raw-ledger の英語 schema 値** (actor/action: system/inputter/intake…): §3.7 sanctioned 例外 (監査機械可読性)、見出しのみ和訳候補は R1。
>   - **literal background `inert` 以外の高度 modal a11y (portal 化等)**: focus-trap + 背景 inert + focus 移動 で G5 gate (scroll-lock + dirty-dismiss) と open-modal axe を満たし、portal refactor は不要不急ゆえ defer。
>   - **axe color-contrast**: jsdom 非評価ゆえ 15-route smoke は構造 a11y のみ。contrast は AR1/R7 token + check-design (R7) が別途担保 (over-claim しない)。
> - **gate 機械確認**: check:all green (test 212) / check-design 違反 0 (R7/icon/emoji/future-date) / typology 15 全 recall surface 一致 (current-lock=14 残存 0) / active-src 2026-05-31 grep 0。
...
|---|---|
| **9 画面 lock** (ia-overview-v2 / screen-contracts / coverage-matrix) | **P1-2 で 9→11 (検索/通知、W2b ✓)、P1-3 (IA scope=(a)) で 11→14 (業務責任者 landing + config-approvals + escalations の 3 画面)、W3 C4 で 14→15 (`/cases/new` 手動起票 = B)**。typology lock を **A×3/B×9/C×3=15** へ更新 (W2b で expansion note 付与済、W2c で 14 relock、W3 で 15 relock) |
