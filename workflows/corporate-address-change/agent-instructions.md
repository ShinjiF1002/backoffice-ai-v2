...
- **staging knowledge は AI runtime に visible だが weight low** (信頼度低、未承認): 参照可、ただし compiled より優先度低 (DOC-FW-01 §3.3)
...
| 中     | `knowledge/staging/*.md` (`category != data_error`)             | medium | reviewed staging、citation 注意付き            |
| 低     | `knowledge/staging/*.md` (`category != data_error`)             | low    | unreviewed staging、citation せず              |
...
代わりに Audit Trail (`/audit`) に記録、Manual 管理者が定期的に review (DOC-FW-01 §2.3、DOC-KNW-04 で詳述予定)。
...
- DOC-KNW-04 (Day 8 起稿予定): AI 日次分析 logic SSOT / 5-category routing 詳細 / LLMOps framework
