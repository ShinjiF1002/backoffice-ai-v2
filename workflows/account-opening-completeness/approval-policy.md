...
`data_error` 以外の差戻しコメントは staging knowledge として AI runtime に visible になる (DOC-FW-01 §3)。`data_error` は staging に記録するが、AI runtime citation 対象外とし、log / audit / 別 routing で扱う (DOC-WF-account-opening-agent-instructions §2.2、DOC-KNW-04 で詳述予定)。
