# Backoffice AI v2 — Cloud Architecture (Phase 1 hand-off Draft、v2.4 current lock)
...
> **v2.0 → v2.3.2 pivot 概要**: v1.0-v1.3 (Tokyo + Osaka 前提) から us-east-1 + us-west-2 に pivot、規制 framework は FISC + 個情法 + 銀行法 + 犯収法 → **NYDFS Part 500 + FRB SR 11-7 + OCC SR 11-7 + BSA-AML + OFAC + GLBA Reg P + FFIEC AIO + State law (NY SHIELD / CCPA-CPRA / VA / CO / CT / UT)** に swap。Bedrock Claude Sonnet 4.6 / Haiku 4.5 は us-east-1 + us-west-2 で In-Region: Yes、v1.2 ADR-4 3 択 gate は Option A trivially closure (v2.2 で historical demotion 完了、v2.3.1 で metadata sync)。JP parent (本店) への報告 / cross-border data flow は別 doc (DOC-CA-09 candidate、本 doc scope 外)。

| 項目            | 値                                                                                                                                                                                                                                                                            |
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 文書 ID         | DOC-CA-08                                                                                                                                                                                                                                                                     |
| 文書名          | Cloud Architecture (8 層 + Security + DR + Cost + ADR、AWS us-east-1 + us-west-2、current v2.3.2 lock for JP 銀行 America division)                                                                                                                                         |
| 版数            | v2.4 (autonomous prod-ready loop Cycle 7: §14.6 FinOps governance + §14A Tenant onboarding/offboarding SOP)                                                                                                                                                                  |
...
- **DM-07 との関係**: DM-07 (current v1.6.2、v1.4 で US pivot 導入) は **persistence layer (Aurora PG 16 / S3 Object Lock / KMS / OpenSearch / pgvector) + US regulatory mapping** の SSOT。本 doc は **その上層 8 層** (Identity / Network / Compute / AI Runtime / Integration / Observability / CI-CD / Frontend) + 横断 layer (Security / DR / Cost) を SSOT 化
...
- **推奨 stack 結論先出し**: AWS **us-east-1 (3 AZ) + us-west-2 (DR、3 AZ)**、**Lambda + Fargate + Step Functions + Bedrock Claude Sonnet 4.6 + Haiku 4.5 (us-east-1 + us-west-2 In-Region available、v2.0 ADR-4 closure) + Computer Use ECS sandbox + API Gateway + Cognito (federated to Entra ID) + CDK TypeScript + GitHub Actions → CodePipeline + Liquibase**。詳細 ADR §13
- **regulatory framework (introduced in v2.0、current v2.3.2)**: **Federal** (FRB SR 11-7 / OCC SR 11-7 + 2023-17 / FFIEC IT Examination Handbook + AIO Booklet / BSA-AML (FinCEN) + USA PATRIOT 326 CIP / OFAC sanctions / GLBA + Reg P / Safeguards Rule / SOX / CFPB) + **State** (NYDFS 23 NYCRR Part 500 / NY SHIELD Act / CCPA-CPRA / VA-CDPA / CO Privacy Act / CT Data Privacy Act / UT Consumer Privacy Act / IL BIPA / WA My Health My Data Act)。State 適用は実 operating state に依存、Phase 1 で external legal counsel が finalize
...
4. **US 規制 framework を逐条充足する security baseline** (introduced in v2.0、current v2.3.2): NYDFS 23 NYCRR Part 500 + FRB SR 11-7 + OCC SR 11-7 + 2023-17 + FFIEC IT Examination Handbook + BSA-AML (FinCEN) + OFAC + GLBA + Reg P + Safeguards Rule + SOX + State law (NY SHIELD / CCPA-CPRA / 等)。SCP / VPC endpoints / KMS / WAF / GuardDuty / Macie / CloudTrail org-wide を default-on、root account でも tamper 不能な audit chain (DM-07 §9) と整合
...
- **AZ (introduced in v2.0、current v2.3.2)**: Availability Zone。us-east-1 (Virginia) は 6 AZ (a/b/c/d/e/f)、本 doc は a/b/c の 3 AZ 使用。us-west-2 (Oregon) は 4 AZ (a/b/c/d)、本 doc は a/b/c の 3 AZ 使用
...
- **In-region (introduced in v2.0、current v2.3.2)**: PII / customer data を扱う Bedrock invoke は **us-east-1 + us-west-2 で In-Region available な model** を使用 (Claude Sonnet 4.6 / Haiku 4.5 共に両 region で In-Region: Yes、AWS 公式 model card §19.1 で verify 済)。v1.2 の 3 択 gate は v2.0 で Option A trivially closure。Cross-region inference (us-east-1 / us-west-2 以外) は §12.1 SCP で deny
...
| L4  | AI Runtime               | Bedrock Claude Sonnet 4.6 + Haiku 4.5 (introduced in v2.0、current v2.3.2: us-east-1 + us-west-2 In-Region available standard、ADR-4 closure) / Bedrock Knowledge Bases (Phase 2 候補) / Computer Use via Claude API | §7      |
...
| L4 AI Runtime | Bedrock Claude Sonnet 4.6 + Haiku 4.5 (introduced in v2.0、current v2.3.2: us-east-1 + us-west-2 In-Region standard、ADR-4 closure)、Anthropic 直 API は cross-cloud egress 禁止 (別問題) | §13 ADR-4 (introduced v2.0、current v2.3.2) |
...
- **高権限 role の phishing-resistant MFA (F1、introduced in v2.0、current v2.3.2)**: 承認者 / 業務責任者 / AI 管理者 / Auditor は **FIDO2 / WebAuthn (Windows Hello for Business or hardware key) 必須**、TOTP / SMS は禁止。入力者は TOTP 許容 [仮説 / 要検証、**NYDFS 23 NYCRR Part 500.12 MFA 要件**と整合確認、open question §17 #16]
...
### 5.1 VPC topology (us-east-1 primary、introduced in v2.0、current v2.3.2)
...
- `*.gsi.go.jp` (国土地理院 API)
- `*.bedrock-runtime.us-east-1.amazonaws.com` + `*.bedrock-runtime.us-west-2.amazonaws.com` (Bedrock、ただし VPC endpoint で先に capture、introduced in v2.0、current v2.3.2)
...
- **CMK 配置 (introduced in v2.0、current v2.3.2)**: us-east-1 primary + us-west-2 replica (auto-replicate)、key policy は CDK で **両 region に同 JSON apply** (Round 2 critique #5)
- **CMK 用途別 separation**:
...
### 7.1 Bedrock Claude (introduced in v2.0 US pivot、current v2.3.2: In-Region standard)

- **Region (introduced in v2.0 US pivot、current v2.3.2)**: `us-east-1` primary + `us-west-2` DR (両 region で Claude Sonnet 4.6 + Haiku 4.5 In-Region: Yes、AWS 公式 model card primary source §19.1 で verify)、cross-region inference は SCP で deny (`us-east-1` + `us-west-2` のみ allow)。v1.2 の Option A trivially 採用、3 択 gate は v2.0 で closure + v2.2 で historical demotion
- **Model**:
  - **Claude Sonnet 4.6** (primary、AI 入力結果生成 / AI 日次分析 / proposal generation)
  - **Claude Haiku 4.5** (secondary、軽量 task = embedding pre-process / confidence score 計算 / Alert classification)
...
- **Invocation logging**:
  ...
  - **S3 (KMS-encrypted, Object Lock Governance)** に prompt + response full payload を archive、retention は customer_pii class (DM-07 current v1.6.2 §9.4 US framework) と同期 = 5 年 (kyc_document case も BSA Section 1010.430 で 5 年継承、introduced in v1.4 US pivot、current v1.6.2 で 7 年 → 5 年に swap)
...
async function processCase(caseId: string) {
  ...
  // 4. Bedrock Claude invoke (PII data → us-east-1 + us-west-2 In-Region only、v2.2 ADR-4 active decision)
...
### 7.5 Cross-region inference 禁止 SCP
...
DR で us-west-2 移行時のみ allow (introduced in v2.0、current v2.3.2 で両 region default allow、SCP は両 region のみ allow + outside Deny)。
...
- **WAF (introduced in v2.0、current v2.3.2)**: AWSManagedRulesCommonRuleSet + RateBasedRule + Geo restriction (**US only (Phase 1)**、operating state 拡大時に Canada / 他 NA を追加候補、open question §17 #29)
...
CloudTrail は AWS control plane の event のみ、業務 system DB の SELECT 内容 (= 顧客 PII を取得しているか) は AWS 側から観測不能。**GLBA Safeguards Rule + NYDFS 500.06 audit trail + FFIEC IT Examination Handbook access management** を満たすため (introduced in v2.0、current v2.3.2)、2 段化:
...
- **両 region deploy (introduced in v2.0、current v2.3.2)**: `cdk deploy --all` for `us-east-1` and `us-west-2` を CI で並列実行、drift check 必須 (Round 3 #6)
...
正規化対象: `us-east-1` / `us-west-2` → `REGION` (introduced in v2.3、current v2.3.2、historical: `ap-northeast-{1,3}` は v1.x phase の Tokyo+Osaka 前提)、AZ ID (`us-east-1a/b/c` / `us-west-2a/b/c`) → `AZ`、Account ARN suffix → `ACCOUNT`、KMS key ID → `KEY_ID`。**正規化後 0 diff** が gate。
...
| Deny `bedrock:InvokeModel*` outside `us-east-1` + `us-west-2` (introduced in v2.0、current v2.3.2、両 region で In-Region available のため Option A trivially closure) | §7.5 + ADR-4 (introduced v2.0、current v2.3.2) |
...
| Deny all AWS API except from `aws:SourceVpc` in [VPC-us-east-1, VPC-us-west-2] (except CloudFront / Route 53 control plane and IAM Identity Center auth) — F19 (introduced in v2.0、current v2.3.2) | 外部からの credential abuse 防御 |
| Deny `kms:Decrypt` / `kms:GenerateDataKey` on `cmk-aurora-prod` / `cmk-bedrock-log` outside `us-east-1` + `us-west-2` (introduced in v2.0、current v2.3.2) | KMS key data 持ち出し防御 |
...
Bedrock InvokeModel (prod account)
  → Bedrock invocation logging configuration:
     ...
     - S3: full payload (prompt + response) を **security account** の bucket に cross-account write:
        ...
        - retention: customer_pii class 5yr / kyc_document case 5yr 継承 (introduced in v1.4 US pivot、DM-07 current v1.6.2、historical: pre-v1.4 は 7yr、BSA Section 1010.430 baseline で 5yr に変更)
        - CRR to us-west-2
...
### ADR-2: VPC topology = private subnet + VPC endpoints default、NAT GW 限定 (introduced in v2.0、current v2.3.2)

- **Status**: Accepted (introduced in v2.0 US pivot で region rename + JP parent peering CIDR reserve 追加、current v2.3.2)
...
### ADR-4: AI Runtime = Bedrock Claude in-region (us-east-1 + us-west-2、introduced in v2.0、current v2.3.2 採用、v2.2 で active decision に simplify)

- **Status (v2.0/v2.2)**: **Accepted** — US deploy 前提により Claude Sonnet 4.6 / Haiku 4.5 が us-east-1 + us-west-2 で In-Region: Yes、本 decision は **active single choice** (v1.2 で記述した 3 択 gate は v2.0 US pivot により superseded、§ADR-4.HIST に historical trace として保存)
- **Context**: PII / customer data を含む prompt の data residency。US deploy + JP 銀行 America division 前提では us-east-1 (NY ops から ~10ms) + us-west-2 (DR pair) の Bedrock In-Region invoke が standard
- **Primary source (2025-Q4 時点)**: AWS 公式 model card で Claude Sonnet 4.6 / Haiku 4.5 が **us-east-1 + us-west-2 で In-Region: Yes** を verify (Phase 1 hand-off 時に再 verify、open question §17 #3)
...
- **Decision (v2.2 active)**: Bedrock Claude Sonnet 4.6 (primary、AI 入力 / 日次分析 / proposal) + Haiku 4.5 (secondary、embedding / lightweight) を **us-east-1 (primary) + us-west-2 (DR)** で invoke、cross-region inference は SCP で両 region のみ allow + outside Deny (§12.1)
...
- **Consequences**:
  - Phase 1 hand-off 時に AWS 公式 model card の us-east-1 + us-west-2 In-Region: Yes status を再 verify (§16 pre-flight #3、open question §17 #3 と同期)
  - 新 Claude model GA 時の "US region 対応 lag" risk は 0-1 month 想定 (Tokyo lag より小さい、introduced in DM-07 v1.4 US pivot、current DM-07 v1.6.2 で大幅低下、§18 R1 と整合)
  - Bedrock invocation log cross-account immutability (§12.4) は In-Region invoke で完結、Global routing carve-out 不要
...
- **Decision**: 5 CMK (`cmk-aurora-prod` / `cmk-s3-audit` / `cmk-s3-evidence` / `cmk-bedrock-log` / `cmk-secrets`)、multi-Region key で us-east-1 + us-west-2 自動 replicate (introduced in v2.0、current v2.3.2)、policy は CDK で両 region 同 JSON apply (CI で diff gate)
...
- **Decision (introduced in v2.0、current v2.3.2)**: AWSManagedRulesCommonRuleSet + AWSManagedRulesKnownBadInputsRuleSet + AWSManagedRulesAmazonIpReputationList + Bot Control (Common) + RateBasedRule (per-IP 2000/5min) + Geo restriction (**US only (Phase 1)**、operating state 拡大時 NA 追加)
...
| **合計 (Phase 1 us-east-1 + us-west-2、introduced in v2.0、current v2.3.2)**  | **~$1,700**       | **~$5,100**        | **~$13,500**         | v2.0 で US region pricing 適用 (~10-15% 安、ただし inter-region us-east↔us-west $0.02/GB は Tokyo↔Osaka より安価)、v1.1 試算 $1,900/$5,700/$15,000 から下方修正 |
...
- **Phase 1 (introduced in v2.0、current v2.3.2)**: Aurora Global DB (us-east-1 writer + us-west-2 reader)、DR 発動時 manual promote
...
| **CDK cross-region drift** (Round 3 #6)            | 月次 (CI)  | us-east-1 / us-west-2 stack diff = 0 (introduced in v2.0、current v2.3.2)                                |
...
2. **US 規制 framework AWS service mapping doc 逐条 review** (introduced in v2.0 US pivot、current v2.3.2): Bedrock / Cognito / Aurora Global DB / Object Lock 等の AWS service が **NYDFS 23 NYCRR Part 500 (500.02 / 500.06 / 500.12 / 500.15 / 500.17 等) + FRB SR 11-7 + OCC SR 11-7 + 2023-17 + FFIEC IT Examination Handbook + BSA-AML + OFAC + GLBA + Safeguards Rule + State law (NY SHIELD / CCPA-CPRA / 等)** 各条 control に充足することを external counsel + Compliance officer + 外部監査と確認
3. **Bedrock In-Region availability primary source 再 verify (introduced in v2.0 US pivot、current v2.3.2 で Option A trivially closure 済) + concurrent invoke quota 確認**: AWS 公式 model card を hand-off 時点で再 read (Claude Sonnet 4.6 + Haiku 4.5 が us-east-1 + us-west-2 で In-Region: Yes が継続していることを確認)、Phase 1 想定 token 量を sandbox で sustain 実測 + service quota increase 申請
...
| 16 | NYDFS 23 NYCRR Part 500.12 MFA 要件の高権限 user 認証強度 mapping (F1 追加、introduced in v2.0、current v2.3.2 で FISC 第 13 版 から swap) — TOTP vs WebAuthn 各 role 別認証要件の正式確認 | Security 関係者 + 外部監査 (external counsel)         | Phase 1 設計 gate                  |
...
| 18 | Bedrock Guardrails us-east-1 + us-west-2 GA availability + prompt injection detection performance 確認 (F10 追加、introduced in v2.0、current v2.3.2) | AI 管理者                                             | Phase 1 設計 gate                  |
...
| #  | リスク                                                                                            | mitigation                                                                                                                                        |
| -- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1 | Bedrock us-east-1 + us-west-2 model availability lag (新 Claude version GA から US region 対応まで通常 0-1 month、introduced in v2.0、current v2.3.2 で large lag risk 大幅低下) | Bedrock model version pin (DM-07 §3.2)、cutover は Type B 設定承認、US 未対応中は staging で外部 sandbox 評価のみ |
...
| R3 | Aurora Global DB の write-forwarding 制約 (introduced in v2.0、current v2.3.2)                                       | DR 時のみ us-west-2 に writer promote、平常時は read-only replica として monitor (DM-07 §14 R10 と同)                                              |
| R4 | KMS multi-Region key policy drift (us-east-1 / us-west-2 で sync 漏れ、introduced in v2.0、current v2.3.2)            | CDK で両 region に同 JSON apply + CI で `aws kms describe-key` diff gate (Round 2 #5)                                                            |
...
| R10 | DR 発動時の Cognito session continuity (us-east-1 → us-west-2、introduced in v2.0、current v2.3.2)                    | Cognito User Pool は region 単位、Phase 1 では Cognito も両 region に deploy + Route 53 failover、user re-login 受容 + employee_ref-based SoD continuity (§15.3 F20)                              |
| **R11** (~~v1.2 P0~~、v2.0 で closure)| ~~Bedrock Claude In-Region 不在による data residency assumption 破綻~~ — v2.0 US pivot により closure (us-east-1 + us-west-2 で In-Region: Yes) | - |
...
- **DM-07** (`docs/07-data-model.md` current v1.6.2、persistence foundation、最重要 dependency)
...
### 19.1 外部 reference (primary source、introduced in v2.0、current v2.3.2 で US 規制 reference 追加)

**AWS service primary source (v1.2 + v2.0)**:
- **AWS Bedrock model card — Claude Sonnet 4.6**: https://docs.aws.amazon.com/bedrock/latest/userguide/model-card-anthropic-claude-sonnet-4-6.html (us-east-1 / us-west-2 In-Region: Yes、ap-northeast-1 In-Region: No が v1.2 で検出 → v2.0 US pivot で resolved)
...
- AWS Bedrock Cross-region inference documentation (introduced in v2.0 US pivot、current v2.3.2 では参照頻度低下、In-Region で完結)
