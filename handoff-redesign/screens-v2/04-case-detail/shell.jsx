// Chrome + primitives for CaseDetail hi-fi mockup.
// Exports to window: Sidebar, TopBar, CaseHeader, CaseFooter, LifecycleStepper,
//                    StatusBadge, FilterChip, MetaChip, Btn, Icon, Field,
//                    Disclosure, EmptyState.
const { useState, useEffect, useMemo, useRef } = React;

/* ---------- icons (1.5px stroke, fg) ---------- */
function Icon({ name, size = 14, color = 'currentColor', strokeWidth = 1.5, style }) {
  const props = { width: size, height: size, viewBox: '0 0 24 24', fill: 'none', stroke: color, strokeWidth, strokeLinecap: 'round', strokeLinejoin: 'round', style };
  switch (name) {
    case 'hub':       return <svg {...props}><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/></svg>;
    case 'inbox':     return <svg {...props}><path d="M3 13l2.5-7A2 2 0 0 1 7.4 5h9.2a2 2 0 0 1 1.9 1.4L21 13"/><path d="M3 13h5l1.5 2.5h5L16 13h5"/><path d="M3 13v5a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-5"/></svg>;
    case 'sparkles':  return <svg {...props}><path d="M12 3l1.7 4.6L18.3 9 13.7 10.7 12 15.3 10.3 10.7 5.7 9l4.6-1.4z"/><path d="M19 14v3"/><path d="M17.5 15.5h3"/><path d="M5 17v3"/><path d="M3.5 18.5h3"/></svg>;
    case 'agent':     return <svg {...props}><circle cx="12" cy="8" r="3.2"/><path d="M5 20a7 7 0 0 1 14 0"/></svg>;
    case 'eye':       return <svg {...props}><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12z"/><circle cx="12" cy="12" r="3"/></svg>;
    case 'chevR':     return <svg {...props}><path d="M9 6l6 6-6 6"/></svg>;
    case 'chevD':     return <svg {...props}><path d="M6 9l6 6 6-6"/></svg>;
    case 'alert':     return <svg {...props}><path d="M12 4l10 17H2L12 4z"/><path d="M12 10v4"/><circle cx="12" cy="18" r="0.6" fill="currentColor"/></svg>;
    case 'check':     return <svg {...props}><path d="M5 13l4 4L19 7"/></svg>;
    case 'check-circle':return <svg {...props}><circle cx="12" cy="12" r="9"/><path d="M8.5 12.5l2.5 2.5 4.5-5"/></svg>;
    case 'x':         return <svg {...props}><path d="M6 6l12 12M6 18L18 6"/></svg>;
    case 'doc':       return <svg {...props}><path d="M14 3H7a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V8z"/><path d="M14 3v5h5"/><path d="M8 13h8M8 17h6"/></svg>;
    case 'search':    return <svg {...props}><circle cx="11" cy="11" r="6"/><path d="M16 16l4 4"/></svg>;
    case 'bell':      return <svg {...props}><path d="M6 16V11a6 6 0 0 1 12 0v5l1.5 2H4.5L6 16z"/><path d="M10.5 21a1.5 1.5 0 0 0 3 0"/></svg>;
    case 'arrowRight':return <svg {...props}><path d="M5 12h14M13 6l6 6-6 6"/></svg>;
    case 'edit':      return <svg {...props}><path d="M4 20h4l11-11-4-4L4 16v4z"/><path d="M14 6l4 4"/></svg>;
    case 'reload':    return <svg {...props}><path d="M3 12a9 9 0 0 1 15.5-6.3L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-15.5 6.3L3 16"/><path d="M3 21v-5h5"/></svg>;
    case 'esc':       return <svg {...props}><path d="M12 4v14"/><path d="M6 10l6-6 6 6"/></svg>;
    case 'shield':    return <svg {...props}><path d="M12 3l8 3v6c0 5-3.5 8.5-8 9-4.5-.5-8-4-8-9V6l8-3z"/><path d="M9 12l2 2 4-4"/></svg>;
    case 'pencil-dot':return <svg {...props}><path d="M4 20h4l11-11-4-4L4 16v4z"/><circle cx="18" cy="6" r="1" fill="currentColor"/></svg>;
    default: return null;
  }
}

/* ---------- StatusBadge / FilterChip / MetaChip primitives ---------- */
function StatusBadge({ children, tone = 'slate', style }) {
  return <span className="chip status" data-tone={tone} style={style}>{children}</span>;
}
function FilterChip({ children, pressed = false, onClick, style }) {
  return <button className="chip filter" aria-pressed={pressed} onClick={onClick} style={style}>{children}</button>;
}
function MetaChip({ children, tone, style, icon }) {
  return (
    <span className="chip meta" data-tone={tone} style={style}>
      {icon && <Icon name={icon} size={12} />}
      {children}
    </span>
  );
}

/* ---------- Btn ---------- */
function Btn({ children, variant = 'default', size = 'md', disabled, tooltip, icon, iconRight, onClick, style, type, autoFocus }) {
  const cls = ['btn'];
  if (variant === 'primary') cls.push('btn-primary');
  if (variant === 'danger')  cls.push('btn-danger');
  if (variant === 'ghost')   cls.push('btn-ghost');
  if (size === 'sm') cls.push('btn-sm');
  if (size === 'xs') cls.push('btn-xs');
  const extra = {};
  if (disabled && tooltip) extra['data-tooltip'] = tooltip;
  return (
    <button
      type={type || 'button'}
      className={cls.join(' ')}
      aria-disabled={disabled || undefined}
      onClick={disabled ? undefined : onClick}
      autoFocus={autoFocus}
      style={style}
      {...extra}
    >
      {icon && <Icon name={icon} size={size === 'xs' ? 12 : 14} />}
      {children}
      {iconRight && <Icon name={iconRight} size={size === 'xs' ? 12 : 14} />}
    </button>
  );
}

/* ---------- Disclosure ---------- */
function Disclosure({ open, onToggle, header, children, level = 'L2', count, style }) {
  return (
    <div style={{ ...style }}>
      <button
        onClick={onToggle}
        style={{
          display: 'flex', alignItems: 'center', gap: 6,
          width: '100%', textAlign: 'left',
          background: 'transparent', border: 'none', padding: '6px 0',
          color: 'var(--fg-muted)',
          fontSize: 12, fontWeight: 500,
        }}
      >
        <Icon name="chevR" size={12} className="disclosure-caret" style={{ transform: open ? 'rotate(90deg)' : 'none', transition: 'transform 200ms var(--ease)' }} />
        <span style={{ color: 'var(--fg)' }}>{header}</span>
        {count != null && <span className="muted">({count})</span>}
        <span style={{ marginLeft: 'auto', fontSize: 10, color: 'var(--fg-subtle)' }}>{level}</span>
      </button>
      {open && <div className="fade-in">{children}</div>}
    </div>
  );
}

/* ---------- LifecycleStepper ---------- */
function LifecycleStepper({ current = 2, style }) {
  const steps = [
    { k: 'intake',    label: '受付',         t: '09:00' },
    { k: 'ai',        label: 'AI 処理',       t: '09:02' },
    { k: 'input',     label: '入力者確認',    t: '進行中' },
    { k: 'checker',   label: '承認者承認',    t: '—' },
    { k: 'reflect',   label: '反映',          t: '—' },
  ];
  return (
    <ol style={{ display: 'flex', alignItems: 'center', gap: 0, margin: 0, padding: 0, listStyle: 'none', ...style }}>
      {steps.map((s, i) => {
        const done = i < current;
        const here = i === current;
        return (
          <li key={s.k} style={{ display: 'flex', alignItems: 'center', flex: i === steps.length - 1 ? '0 0 auto' : 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: '0 0 auto' }}>
              <span style={{
                width: 18, height: 18, borderRadius: 9,
                border: '1.5px solid ' + (done || here ? 'var(--primary)' : 'var(--border-strong)'),
                background: done ? 'var(--primary)' : here ? 'var(--panel)' : 'var(--panel)',
                display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                flex: '0 0 auto',
              }}>
                {done && <Icon name="check" size={11} color="#fff" strokeWidth={2.4} />}
                {here && <span style={{ width: 8, height: 8, borderRadius: 4, background: 'var(--primary)' }} />}
              </span>
              <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.25 }}>
                <span style={{ fontSize: 12, fontWeight: here ? 600 : 500, color: here ? 'var(--primary-hover)' : done ? 'var(--fg)' : 'var(--fg-muted)' }}>{s.label}</span>
                <span className="caption mono" style={{ fontSize: 10 }}>{s.t}</span>
              </div>
            </div>
            {i < steps.length - 1 && (
              <span style={{
                flex: 1, height: 0,
                borderTop: '1.5px ' + (done ? 'solid var(--primary)' : 'dashed var(--border-strong)'),
                margin: '0 12px',
                minWidth: 24,
              }} />
            )}
          </li>
        );
      })}
    </ol>
  );
}

/* ---------- Sidebar ---------- */
function Sidebar() {
  const nav = [
    { k: 'hub',         label: 'ハブ',           icon: 'hub' },
    { k: 'queue',       label: '受信トレイ',     icon: 'inbox', active: true },
    { k: 'proposals',   label: 'AI 提案レビュー', icon: 'sparkles' },
    { k: 'agents',      label: 'Agent 設定',     icon: 'agent' },
    { k: 'observatory', label: 'モニタリング', icon: 'eye' },
  ];
  return (
    <aside style={{
      gridArea: 'side',
      background: 'var(--panel)',
      borderRight: '1px solid var(--border)',
      display: 'flex', flexDirection: 'column',
      position: 'sticky', top: 0, height: '100vh',
    }}>
      <div style={{ padding: '14px 14px 12px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{
            width: 26, height: 26, borderRadius: 6,
            background: 'var(--fg)', color: '#fff',
            display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            fontWeight: 700, fontSize: 12,
          }}>B</div>
          <div className="sidebar-label" style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.2 }}>
            <span style={{ fontWeight: 700, fontSize: 13 }}>Backoffice AI</span>
            <span className="caption mono" style={{ fontSize: 9, color: 'var(--fg-subtle)' }}>v2 prototype</span>
          </div>
        </div>
      </div>
      <nav style={{ flex: 1, padding: '10px 8px', overflow: 'auto' }}>
        {nav.map((n, i) => (
          <a key={n.k} href="#" style={{
            display: 'flex', alignItems: 'center', gap: 10,
            padding: '8px 10px',
            borderRadius: 6,
            color: n.active ? 'var(--primary-hover)' : 'var(--fg)',
            background: n.active ? 'var(--primary-soft)' : 'transparent',
            textDecoration: 'none',
            fontSize: 13, fontWeight: n.active ? 600 : 500,
            marginBottom: 2,
          }}>
            <Icon name={n.icon} size={16} />
            <span className="sidebar-label">{n.label}</span>
          </a>
        ))}
      </nav>
      <div style={{ padding: 10, borderTop: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 8px' }}>
          <div style={{
            width: 28, height: 28, borderRadius: 14,
            background: 'var(--panel-inset)', color: 'var(--fg)',
            display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            fontWeight: 600, fontSize: 11,
          }}>山田</div>
          <div className="sidebar-label" style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.25, minWidth: 0 }}>
            <span style={{ fontSize: 12, fontWeight: 600 }}>山田太郎</span>
            <span className="caption" style={{ fontSize: 10 }}>入力者</span>
          </div>
        </div>
      </div>
    </aside>
  );
}

/* ---------- TopBar ---------- */
function TopBar() {
  return (
    <div className="sticky-top" style={{
      display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      height: 44, padding: '0 16px',
      borderBottom: '1px solid var(--border)',
      background: 'var(--panel)',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <span className="caption">業務</span>
        <button style={{
          display: 'inline-flex', alignItems: 'center', gap: 8,
          height: 28, padding: '0 10px',
          border: '1px solid var(--border-strong)',
          borderRadius: 6,
          background: 'var(--panel)',
          fontSize: 13, fontWeight: 600, color: 'var(--fg)',
        }}>
          法人住所変更
          <Icon name="chevD" size={12} color="var(--fg-muted)" />
        </button>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <Icon name="search" size={16} color="var(--fg-subtle)" aria-hidden />
        <Icon name="bell"   size={16} color="var(--fg-subtle)" aria-hidden />
        <PrototypeModeLabel />
        <div style={{
          width: 28, height: 28, borderRadius: 14,
          background: 'var(--panel-inset)', color: 'var(--fg)',
          display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
          fontWeight: 600, fontSize: 11,
        }}>山田</div>
      </div>
    </div>
  );
}

function PrototypeModeLabel() {
  return (
    <span
      data-tooltip="本プロトタイプはメモリ上のモック状態です。永続化なし / 外部システム未接続 / 実顧客データ未使用 / 実規制の引用なし"
      style={{
        display: 'inline-flex', alignItems: 'center', gap: 6,
        height: 22, padding: '0 8px',
        borderRadius: 6,
        background: 'var(--panel-inset)',
        color: 'var(--fg-muted)',
        fontSize: 11, fontWeight: 500,
      }}
    >
      プロトタイプ表示 — 外部システム未接続 / 証跡はモック
    </span>
  );
}

/* ---------- CaseHeader (sticky top, min-height 88px) ---------- */
function CaseHeader({ mode, setMode, openCount }) {
  return (
    <header
      className="sticky-top"
      style={{
        minHeight: 88,
        padding: '14px 24px 12px',
        background: 'var(--panel)',
        borderBottom: '1px solid var(--border)',
        top: 44,
      }}
    >
      {/* Breadcrumb */}
      <div className="caption" style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
        <a href="#" style={{ color: 'var(--fg-muted)', textDecoration: 'none' }}>法人住所変更</a>
        <Icon name="chevR" size={10} color="var(--fg-subtle)" />
        <a href="#" style={{ color: 'var(--fg-muted)', textDecoration: 'none' }}>案件一覧</a>
        <Icon name="chevR" size={10} color="var(--fg-subtle)" />
        <span className="mono" style={{ color: 'var(--fg)', fontWeight: 500 }}>CASE-2026-0142</span>
      </div>

      {/* Title row */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, minWidth: 0 }}>
          <h1 className="h1" style={{ margin: 0, display: 'inline-flex', alignItems: 'baseline', gap: 10 }}>
            <span className="mono" style={{ fontSize: 16, fontWeight: 600 }}>CASE-2026-0142</span>
            <span>法人住所変更</span>
          </h1>
          <StatusBadge tone="slate">入力者確認待ち</StatusBadge>
          <span className="caption">
            受付 <span className="mono">2026-05-31 09:00</span> · 担当 <strong style={{ color: 'var(--fg)', fontWeight: 500 }}>山田太郎</strong>
          </span>
        </div>
        <ModeSegmented mode={mode} setMode={setMode} />
      </div>

      <LifecycleStepper current={2} />
    </header>
  );
}

function ModeSegmented({ mode, setMode }) {
  return (
    <div className="seg" role="tablist" aria-label="画面切替">
      <button role="tab" aria-pressed={mode === 'input'} onClick={() => setMode('input')}>入力者</button>
      <button role="tab" aria-pressed={mode === 'checker'} onClick={() => setMode('checker')}>承認者</button>
    </div>
  );
}

/* ---------- CaseFooter ---------- */
function CaseFooter({ mode, openCount, onApprove, onSendback }) {
  const hasOpen = openCount > 0;
  if (mode === 'input') {
    return (
      <footer className="sticky-bottom" style={{
        padding: '10px 24px',
        borderTop: '1px solid var(--border)',
        background: 'var(--panel)',
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      }}>
        <div className="caption" style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
          {hasOpen ? (
            <>
              <Icon name="alert" size={14} color="var(--alert-soft-fg)" />
              <span style={{ color: 'var(--alert-soft-fg)', fontWeight: 500 }}>
                要確認 <span className="mono">{openCount}</span> 項目を解消してください
              </span>
            </>
          ) : (
            <>
              <Icon name="check-circle" size={14} color="var(--success-soft-fg)" />
              <span style={{ color: 'var(--success-soft-fg)', fontWeight: 500 }}>全項目確認済 — 承認できます</span>
            </>
          )}
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <Btn icon="reload" onClick={onSendback}>差戻し</Btn>
          <Btn
            variant="primary"
            disabled={hasOpen}
            tooltip={hasOpen ? `要確認 ${openCount} 項目を解消してください` : undefined}
            icon="check"
            onClick={onApprove}
          >
            承認
          </Btn>
        </div>
      </footer>
    );
  }
  return (
    <footer className="sticky-bottom" style={{
      padding: '10px 24px',
      borderTop: '1px solid var(--border)',
      background: 'var(--panel)',
      display: 'flex', alignItems: 'center', justifyContent: 'space-between',
    }}>
      <div className="caption" style={{ display: 'inline-flex', alignItems: 'center', gap: 10 }}>
        <MetaChip tone="success" icon="shield">承認者 ≠ 入力者</MetaChip>
        <span>入力者 <strong style={{ color: 'var(--fg)', fontWeight: 500 }}>山田太郎</strong> ≠ 承認者 <strong style={{ color: 'var(--fg)', fontWeight: 500 }}>鈴木課長</strong> · 入力者確認 <span className="mono">2026-05-31 11:30</span></span>
      </div>
      <div style={{ display: 'flex', gap: 8 }}>
        <Btn icon="reload" onClick={onSendback}>差戻し</Btn>
        <Btn variant="primary" icon="check" disabled={hasOpen} tooltip={hasOpen ? `要確認 ${openCount} 項目を解消してください` : undefined} onClick={onApprove}>最終承認</Btn>
      </div>
    </footer>
  );
}

/* ---------- EmptyState ---------- */
function EmptyState({ tone = 'inset', icon = 'alert', title, body, action }) {
  const bg = tone === 'error' ? 'var(--error-soft)' : 'var(--panel-inset)';
  const fg = tone === 'error' ? 'var(--error-soft-fg)' : 'var(--fg-muted)';
  return (
    <div style={{
      padding: 32,
      background: bg, borderRadius: 'var(--r-card)',
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
      gap: 10, textAlign: 'center', minHeight: 200,
    }}>
      <Icon name={icon} size={24} color={fg} />
      <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--fg)' }}>{title}</div>
      <div className="caption" style={{ color: fg, maxWidth: 360 }}>{body}</div>
      {action && <div style={{ marginTop: 4 }}>{action}</div>}
    </div>
  );
}

Object.assign(window, {
  Icon, StatusBadge, FilterChip, MetaChip, Btn, Disclosure,
  LifecycleStepper, Sidebar, TopBar, CaseHeader, CaseFooter,
  PrototypeModeLabel, ModeSegmented, EmptyState,
});
