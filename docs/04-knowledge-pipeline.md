...
| SSOT 区分       | AI 日次分析 logic + 5-category routing 詳細 + Staging usage rules (UI 表示 + agent prompt embedding) + Staging lifecycle + Audit evidence event model 15 field + LLMOps framework + Connection control matrix reference の SSOT |
| Evidence Status | N/A (knowledge pipeline 設計のみ。判断基準 / lifecycle 期間 / KPI gate 仮閾値はすべて `[仮値 / 要 Phase 1 検証]`)                                                                                                               |
| 改版履歴        | v0.1 (2026-05-28): 初版作成 (Day 8、Plan v1.4 P0-1 (staging safety) + P1-1 (staging usage rules) + P1-5 (audit event model) + P1-6 (connection control matrix reference) + P1-8 (staging lifecycle) + v1.4.1 Fix 1/Fix 4 反映)  |
...
- §8 Audit evidence event model (15 field)
...
- **頻度**: 日次 (本番想定。cron / scheduled task) `[仮値 / 要 Phase 1 検証]`
...
本 v2 phase ではすべて `[仮値 / 要 Phase 1 検証]`。本番 threshold は Phase 1 で measurement → calibration → governance review。
...
- **SSOT**: §8 Audit evidence event model (15 field、本 doc § 内 SSOT)
...
## 7. Staging lifecycle (Plan v1.4 P1-8) [仮値 / 要 Phase 1 検証]
...
本 v2 phase ではすべて `[仮値 / 要 Phase 1 検証]`。本番期間は Phase 1 で measurement → calibration → governance review。
...
- 個別 event click で 15 field detail panel 展開
