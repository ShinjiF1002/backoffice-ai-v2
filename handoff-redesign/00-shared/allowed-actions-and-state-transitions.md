...
| reflected | 承認者 / 業務責任者 | **訂正** (reversal) | 反映済の内容に誤り + **コメント必須** | sent-back (再処理へ) | `case_reverse` (訂正) |
...
**reversal (W3 C3、前進のみ→可逆)**: 反映済 (reflected) は通常終端だが、承認者/業務責任者 が理由必須で **訂正** / **取消** できる。**訂正・取消とも sent-back (差戻し再処理) へ送る** (ready 直行は反映済 field が確認済のまま 1-click 再反映できる false-success を生むため不採用)。区別は reversal 記録 (kind/理由) + 通知 (kind=reversal) + banner が担う。入力者は不可 (反映を最終確定した承認者の責務)。二重 reversal は block (不可逆 guard)。参照専用の過去案件 (store 非載) は操作不可。
