...
- **国際送金 boundary** (`workflows/international-transfer-boundary/`): 条件付き制限業務 (restricted boundary pack) として boundary spec のみ記述、UI 画面化なし、Dashboard カード化なし。`$10M 相当以上は AI 自動化不可 [仮説 / 要検証]` (high-value threshold、**実閾値は Phase 1 で検証・決定**)。未満はフレームワーク信頼性確認後に限定自動化を検討。**Session 4 表層では「高額・高リスク取引」と抽象化**、具体閾値 (`$10M [仮説]`) は内部 docs のみ (DOC-ROOT-\_SSOT「Session 4 表層表現規範」参照)
