...
### Sidebar (left rail、5 nav)
1. ハブ — `/` (Hub)
2. 受信トレイ — `/queue` (Queue master)
3. AI 提案レビュー — `/proposals/PROP-...` (ProposalDetail)
4. Agent 設定 — `/agents/agent-...` (AgentDetail)
5. モニタリング — `/observatory?tab=audit` (旧「観測」、内部名 Observatory)

CaseDetail (`/cases/:id`) は Queue row click から navigate、sidebar に非表示。
