...
| 改版履歴        | v0.1 (2026-05-28): 初版作成 (Day 8、Plan v1.4 P1-1 + P1-5 (audit event reference) + v1.4 P0-3 / v1.4.1 Fix 3 / Fix 5 反映)                    |
...
9. **Day 11+ 実装メモ**: Wireframe で table layout + filter chip + 状態 badge 配置確定。Hero 起点 (Demo Chapter 1 開始画面)
...
6. **表示要素**: PageHeader、filter chips (workflow / event type / role / date range)、event timeline (icon + event type + actor + timestamp + summary)、expanded event panel (15 field event model from DOC-KNW-04 §8、特に `compiled_knowledge_citation_ids` と `diff_id` link で関連 entity navigation)、**過去 case 関連ルール更新 Alert UI 適用範囲 2** (timeline 上で「YYYY-MM-DD に [手順名] が更新されました (本 case 処理時の版は当時のまま保持)」を inline 表示、§6.2 参照)、Prototype mode label
7. **遷移**: event detail expand、diff_id link → 該当 proposal page、citation_id link → KnowledgeBrowser
8. **mock data 依存**: `mock-audit.ts` (15 field event model schema + sample events) + 全 mock data の cross-reference
9. **Day 11+ 実装メモ**: event model schema (15 field) は `prototype/src/types/audit.ts` で TypeScript type 定義 (DOC-KNW-04 §8 と整合)
...
| `AiProposalPanel`       | citation list + staging hint + Alert UI (§6.1 / §9) | CaseReview、ProposalReview                    |
...
- `mock-audit.ts`: 15 field event model (DOC-KNW-04 §8) + sample events
