...
- 承認者画面は実装せず、`BusinessApprovalChip` + 静的 mock (`demo/static-mocks/business-approval-view.html`、Day 20 で実装予定) で代替
...
`BusinessApprovalView` は `demo/static-mocks/business-approval-view.html` (Day 20 起稿) として静的 mock のみ、React route 対象外。CaseReview footer の `BusinessApprovalChip` click で別タブで開く (Day 14 medium-fi では disabled wrapper title 経由、Day 20 静的 mock 実体化後に enable)。
...
7. **遷移**: 差戻し click → `/cases/:id/comment`、承認 click → state 変更 + Inbox queue 戻る、`BusinessApprovalChip` click → 静的 mock 別タブ (Day 14 medium-fi では disabled wrapper title 経由、Day 20 で enable)
...
- **click action**: `demo/static-mocks/business-approval-view.html` を別タブで開く (静的 mock、design token は本 doc と整合、Day 20 実体化、Day 14 medium-fi では disabled wrapper title 経由)
- 本 prototype 内では実承認 flow は実装しない (mock state 切替のみ)
- a11y: button role、aria-label「業務承認画面を別タブで開く (Day 20 で実装予定)」、Day 14 medium-fi では `disabled` + `aria-disabled="true"` + wrapper `<span title>` で no-op 解消
