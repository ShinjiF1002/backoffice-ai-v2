...
| 改版履歴        | v0.1 (2026-05-29): 初版作成 (Day 9、Plan v1.4 P0-2 (KPI re-framing) + P1-2-a (KPI 各定義) + P1-5 (Audit event reference) + v1.4.1 Fix 2 反映、SLO 仮値表を \_SSOT.md §1.3 から移管、Day 8 deferred "gate" 語を「仮説 gate / target hypothesis」に正式 framing) |
...
「即時」表現は禁止 (Plan v1.4.1 Fix 2)。SLO 表現は「同一セッション内」「当日中」「日次」「次回 AI 処理から」に統一。
...
- 禁止語: 「即時」「リアルタイム」「数秒以内」(real-time guarantee 風表現、`docs/prior-art-map.md` 経由 trace)
...
| sample size | Phase 1 で確定 [仮値 / 要検証]。Session 4 では sample 仮値で説明                                            |
...
| sample size                    | Phase 1 で確定 [仮値 / 要検証]                                                                                                  |
| 集計頻度                       | 日次 + 週次 + 月次                                                                                                              |
...
- Supervised → Checkpoint: 4 KPI 全件で 3 ヶ月以上連続達成 `[仮値 / 要検証]`
- Checkpoint → Autonomous: 4 KPI 全件で 6 ヶ月以上連続達成 + 9 KRI に異常なし `[仮値 / 要検証]`
...
| --- | ------------------- | --------------------------------------------------------- | ------------------------ |
| K1  | AI 入力承認率       | (§4.2.1)                                                  | ≥ 99% `[仮説]`           |
| K2  | 人手上書き率        | (§4.2.2)                                                  | ≤ 1% `[仮説]`            |
| K3  | Alert 発生率        | (§4.2.3)                                                  | ≤ 5% `[仮説]`            |
| K4  | 承認者差戻し率      | (§4.2.4)                                                  | ≤ 1% `[仮説]`            |
| K5  | 案件平均処理時間    | AI 入力開始 → 承認者承認完了までの平均時間                | 業務別に定義 `[仮説]`    |
| K6  | compiled 昇格成功率 | Procedure Update Proposal が業務責任者に approve される率 | ≥ 70% `[仮説]`           |
...
| #   | KRI 名                           | trigger 条件                                                                     | 対応                                         |
| --- | -------------------------------- | -------------------------------------------------------------------------------- | -------------------------------------------- |
| R1  | AI 入力承認率 drift              | target hypothesis (≥ 99%) を週次平均で下回り、2 週連続 `[仮値 / 要検証]`         | Manual 管理者通知、AI 管理者 review          |
| R2  | 人手上書き率 spike               | target hypothesis (≤ 1%) を日次平均で 3 倍以上超過 `[仮値 / 要検証]`             | Manual 管理者通知 + Forced Downgrade 検討    |
| R3  | Alert false positive 急増        | precision (§4.2.3 併記) が週次で 50% 下回り `[仮値 / 要検証]`                    | approval-policy.md Alert 条件 review trigger |
| R4  | 承認者差戻し率 急上昇            | target hypothesis (≤ 1%) を週次平均で 5 倍以上超過 `[仮値 / 要検証]`             | 業務責任者通知 + Forced Downgrade 検討       |
| R5  | UI drift 検知                    | 業務システム画面 layout 変更による RPA / Computer Use 操作失敗 `[仮値 / 要検証]` | AI 管理者通知 + agent-instructions.md review |
| R6  | compiled / staging conflict 発生 | 同一 workflow 内で compiled と staging が矛盾 (DOC-KNW-04 §7.2) `[仮値]`         | Manual 管理者 triage 強制                    |
| R7  | 同種差戻し再発頻度               | 同一 case ID で 2 回以上 sendback `[仮値 / 要検証]`                              | Manual 管理者 + AI 管理者通知                |
| R8  | agent_version drift              | proposal source AI が deprecated agent_version で生成 `[仮値 / 要検証]`          | AI 管理者通知、agent_version 強制 upgrade    |
| R9  | データ品質低下                   | `data_error` category 差戻しが週次平均で 2 倍以上増加 `[仮値 / 要検証]`          | 入力 PDF 受領元への channel review trigger   |

KRI 閾値はすべて `[仮値 / 要検証]`。Phase 1 で measurement → calibration → governance review。
...
| 段階       | 案件承認の介在         | 4 KPI 全件達成期間 `[仮値 / 要検証]` | 9 KRI 状態 |
| ---------- | ---------------------- | ------------------------------------ | ---------- |
