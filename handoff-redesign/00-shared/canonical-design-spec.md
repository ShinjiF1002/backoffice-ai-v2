...
tone semantic: **inset**=中立/受付/system (**却下**を含む、下表参照), **primary**=進行中/AI/確認中, **alert**=要対応/人手介在, **slate**=次段待ち, **success**=完了/良好, **error**=重大/エスカレーション.

> **DC1 解消 (W0、2026-05-30)**: 旧 spec は error semantic に「却下」を含め下表 (却下=inset) + code (`status-tones.ts` proposalStatusToTone:rejected→inset) と矛盾していた。**却下=inset を正**とし error から除去 (却下は「重大」ではなく中立的な処理結果)。
...
| 反映の取消 (reversal/W3 C3) | `Undo2Icon` | 反映の可逆 (reversal banner) | `RotateCcwIcon` |

> 🔴 03-approvals の `pencil-dot` (未定義) は `PencilLineIcon` に置換。emoji (🛡↑✓ 等) は使わない。
> 反映の **訂正** (W3 C3) は 修正(override) と同概念ゆえ `PencilLineIcon` を再利用 (新 icon を増やさない)、**取消** は `Undo2Icon`、可逆化を示す banner は `RotateCcwIcon` (再取得 `RotateCwIcon` とは別 icon)。
...
- icon は lucide-react のみ、**Icon suffix 統一** (inline `<svg>` 0、`pencil-dot` 0、emoji 0、非 suffix import 0)
- **contrast R7 (W0、§9)**: 意味テキストに `fg-subtle` 0 (装飾/disabled/icon/`pointer-events-none` のみ許容)、`fg-tertiary` 以上を使う
- **機械化**: 上記のうち fg-subtle-on-text / Icon-suffix / emoji は `scripts/check-design.mjs` (`npm run check:design`、check:all chain) で grep gate 化。off-token hex 全数 / 全画面 axe は P2B-4 / W3 (§1.0) に残す
...
WCAG AA (通常テキスト 4.5:1) を満たすため、意味テキスト (ユーザーが読む情報) には `fg-subtle` を使わない。

| token | hex | panel #fff | canvas #f8fafc | panel-inset #f1f5f9 | 用途 |
|---|---|---|---|---|---|
| `fg` | #0f172a | ~19:1 | ~18:1 | ~17:1 | 主テキスト |
| `fg-muted` | #64748b | ~4.8:1 (AA) | ~4.6:1 (AA) | ~4.3:1 (**AA 未達**) | 副テキスト (panel/canvas のみ) |
| **`fg-tertiary`** | **#475569** | ~7.6:1 | ~7.4:1 | ~7.1:1 | **意味テキストの最弱 (全背景 AA pass、W0 追加)** |
| `fg-subtle` | #94a3b8 | 2.56:1 | 2.45:1 | 2.34:1 | **装飾/disabled/icon 専用 (意味テキスト禁止)** |

- **例外 (fg-subtle 許容)**: `aria-hidden` icon / disabled・`cursor-not-allowed` / placeholder / `pointer-events-none` / 背景 `bg-`。
- **R7 gate** (`check-design.mjs`): `text-[var(--color-fg-subtle)]` は上記例外マーカーを同一/近接行に持つ場合のみ許容、それ以外は違反 (exit 1)。
- **fg-muted-on-panel-inset**: panel-inset 上の `fg-muted` (4.3:1) は AA 未達。背景文脈依存ゆえ機械 grep ではなく **per-screen 視覚 audit (W1)** で `fg-tertiary`/`fg` へ昇格。

## 10. visual foundation token (W0 定義のみ、per-screen 適用は W1 — functional-first)

W0 では token を `index.css` に**定義のみ**。視覚的な per-screen 適用 (shadow/type scale/density/motion) は W1 の各画面 refine で行い、W0 gate には含めない (functional-first)。

- **elevation**: `--shadow-sm` / `--shadow-md` / `--shadow-lg` + `--shadow-primary-glow` (AI surface ambient、控えめ)。
- **typography scale**: display22 / title18 / subtitle15 / body14 / body-sm13 / caption12 / micro11、max 3 weight、`tnum`。
- **density T1–T4**: per-surface (T1 = 主画面密、T4 = marketing 疎)。
- **motion budget**: T1≤5 / T2≤3 / T3≤2 / T4=0 event、**`prefers-reduced-motion: reduce` で全 animation/transition を global block** (mandatory)。
