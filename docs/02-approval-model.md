...
本 doc 内で Tier 3 規制語 (具体 list は `docs/prior-art-map.md` §規制語 hedge ルール 参照、self-hit 回避のため本 doc には exact list を置かない) を使う時:
...
ai-operator paper では固有部署名 (規制 / 監査 / セキュリティ系) が登場するが、v2 では総称表現に rename:

> ※ 本表の左列 (ai-operator paper の固有部署名) は rename mapping reference として exact 表記、v2 docs / Session 4 表層では事実主張しない。
...
| **Emergency stop**   | Incident (drift / データ漏洩疑い / 異常出力) | R+A: AI 管理者 または Security 関係者 (対応可、事後 co-A)     | 15 分以内 `[仮説 / 要検証]` (`docs/05-metrics-and-gates.md` §3 SLO 仮値表参照、Day 9 移管済) | stop timestamp、発動者、対象 Agent / scope       |
...
| 業務                                 | v2 での扱い                                                                                                                                             | 理由                                                                                        |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| 国際送金                             | `workflows/international-transfer-boundary/` に restricted boundary pack (`automation_status: restricted`、`BOUNDARY.md` §2 の高額閾値以上は AI 自動化不可 `[仮説 / 要検証]`) | 高額・高リスク条件で AI 自動化不可、未満は将来限定自動化検討。実閾値は Phase 1 で検証・決定                                                                                                                          |
...
v2 docs 内では Tier 3 規制語 (具体 list は `docs/prior-art-map.md` §規制語 hedge ルール 参照、self-hit 回避のため本 doc には exact list を置かない) を事実主張せず、上記 prior art pointer のみを使う。Session 4 slide / UI label / mock data には完全に出さない。
