...
| 版数            | v0.13 (autonomous prod-ready loop Cycle 1-12 完了: DM-07 v1.7.2 + CA-08 v2.6 + PFC-09 v0.2 + TM-10 v0.1 + SRE-11 v0.1 + CEM-12 v0.2 の 6 row を Topic mapping に追加 / Cycle 8.5 P0-V Bedrock Geo CRIS + Cycle 10 schema migration + Cycle 11 Geo CRIS cost + Cycle 12 integration sweep で active stale 0 達成)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ステータス      | Draft                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| 国際送金 boundary 仕様 + 条件付き制限 (restricted)                                                    | `workflows/international-transfer-boundary/{workflow.md, BOUNDARY.md, _meta.yaml}` | Day 7    | 3 文書 only、画面化なし、Dashboard カードなし。高額閾値以上で AI 自動化不可 (具体値は `BOUNDARY.md` §2 + `_meta.yaml`)、未満は将来限定自動化検討                          |
...
| **物理データモデル + DB 技術選定 + immutable audit + retention + RLS + row-level encryption + audit chain repair + right-to-erasure resolution + PITR drill + RDS Proxy fail-mode + Liquibase + migration harness** | `docs/07-data-model.md` (DOC-DM-07 v1.7.2)                                          | autonomous prod-ready loop Cycle 5-6 | persistence foundation、Phase 1 hand-off Draft、§5-§13 + §10.10-10.13 (operational depth) |
| **Cloud architecture 12 layer + ADR 13 件 + Security + DR + Cost + FinOps governance + tenant onboarding/offboarding**                                                                                            | `docs/08-cloud-architecture.md` (DOC-CA-08 v2.6)                                    | autonomous prod-ready loop Cycle 7   | upper 8 layer + 4 横断、ADR-1 ~ ADR-13、§14.6 FinOps + §14A Tenant lifecycle |
| **Phase 1 Pre-Flight Execution Checklist (外部 7 PFC item × RACI × acceptance × evidence template × sign-off)**                                                                                                    | `docs/09-pre-flight-execution-checklist.md` (DOC-PFC-09 v0.2)                       | autonomous prod-ready loop Cycle 1   | PFC-01 ~ PFC-07、Type B 設定承認 prerequisite gate の SSOT       |
| **Threat Model (STRIDE × 12 layer × trust boundary + AI/ML 固有 threat + insider + supply chain + key compromise + abuse case)**                                                                                  | `docs/10-threat-model.md` (DOC-TM-10 v0.1)                                          | autonomous prod-ready loop Cycle 2   | 75 threat ID + residual risk register + FRB SR 11-7 接続 |
| **SRE Runbook (SLO/SLI 5 + Error budget policy + S0-S4 severity matrix + on-call rotation + incident response 5 phase + 12 runbook + DR drill SOP)**                                                              | `docs/11-sre-runbook.md` (DOC-SRE-11 v0.1)                                          | autonomous prod-ready loop Cycle 3   | operational readiness SSOT、Phase 1 ops 開始後 daily reference |
| **Compliance Evidence Matrix (NYDFS Part 500 全 23 section + FRB SR 11-7 + OCC + FFIEC + BSA-AML + OFAC + GLBA + SOX + State law 各 control × AWS service mapping × evidence pointer × counsel comment slot)** | `docs/12-compliance-evidence-matrix.md` (DOC-CEM-12 v0.2)                           | autonomous prod-ready loop Cycle 4   | PFC-02 external counsel review input doc                  |
...
v2 docs 内で Tier 3 規制語 (具体 list は `docs/prior-art-map.md` §規制語 hedge ルール 参照、self-hit 回避のため本 doc には exact list を置かない) を使う時:
...
restricted_conditions:
  high_value_threshold: "<金額閾値> [hypothesis_requires_validation]" # 具体値は workflows/{業務}/BOUNDARY.md §2 + _meta.yaml
...
| --------------------------------------------------------------------- | ----------------------------------------------------------------- |
| 国際送金高額閾値 (具体値は `workflows/international-transfer-boundary/BOUNDARY.md` §2 + `_meta.yaml` の `high_value_threshold`、本 SSOT には exact 値を持たない) | 「高額・高リスク取引」(数値は出さない)                            |
...
| 承認者承認画面             | `docs/03-ui-prototype-design.md` (9 画面 spec) vs `docs/06-session4-narrative.md` Slide 3 内 figure (mock)                                           | 03 §7 は `BusinessApprovalChip` 仕様 (CaseReview footer)、06 §4 は mock figure content spec (Slide 3 用)、双方が `demo/static-mocks/business-approval-view.html` (Day 20 実体化) を共通 path として参照 (v1.4 Commit 4 U-8、旧 `業務承認画面` から rename) |
