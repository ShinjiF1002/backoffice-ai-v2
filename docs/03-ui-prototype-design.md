...
9. **Day 12 Page 7 実装注記** (commit `4ffd337`): event model schema (15 行、paired field 含む実 18 field) は `mock-audit.ts` `AuditEvent` interface (camelCase TS + JSDoc snake_case SSOT mapping) で type 定義、DOC-KNW-04 §8.1 と整合。**Layout** (Operational Premium Light + 他 7 page register 共通): Sticky PageHeader (breadcrumb 2-level + h1 + 「直近 30 日 (検証用)」chip + meta「15 項目 schema · paired field 含む実 18」+ workflow filter 3 chip [全業務 / UC-BO-01 / UC-BO-02、useState、aria-pressed、page-global scope]) + SSOT framing 注 (slate-50 inset + History icon、「監査イベントは 15 項目 schema で記録」/ event click で 15 項目 detail 展開) + 監査イベント 時系列 (timestamp DESC sort、各 event row icon + EVENT_TYPE_LABEL + case_id mono + workflow_id mono + 適用範囲 2 chip if rule_update_alert + timestamp + actorLabel mono + summary、`useState expandedId` で 1 row at a time toggle) + 展開時 15-row schema 詳細 (snake_case label mono + JP value、`DetailPanel` + `DetailRow` 内部 component、case_id / workflow_id+version / agent_id+version / prompt/tool/model_config_version / input_artifact_hash / ai_proposal_id / human_decision_id / sendback_category / compiled_knowledge_citation_ids (wide row) / approval_timestamp+approver_id / diff_id / rollback_ref / screenshot_stack_id、note で「Phase 1 実装」「scope-out」明示) + Sticky footer (ダッシュボード戻り link + 検証用 監査機能の拡張は次の実装段階で対応 caption)。**EVENT_TYPE_LABEL** (10 種): system_intake → PDF 受付 / ai_input → AI 入力 / human_review → 入力者確認 / human_sendback → 入力者差戻し / ai_analysis → AI 日次分析 / proposal_approve → 手順承認 / business_approve → 承認者承認 / reflect → 反映 / rule_update_alert → 関連ルール更新 / config_approve → 設定承認。**EVENT_TYPE_STYLE**: icon + badge state-conditional (system/ai = primary, human review = emerald, sendback/rule_update = amber, approve/reflect = emerald)。**適用範囲 2 関連ルール更新 Alert** (DOC-UI-03 §6.2): `rule_update_alert` event type で実装、timeline 上に inline 表示 (banner ではない、row 配下 nested ではなく event row として timestamp 順 inline)、row 全体に amber-50/30 tint + 「適用範囲 2」amber chip、文言「YYYY-MM-DD に [手順名] v0.X が承認されました (本 case 処理時の版は当時のまま 監査証跡 に保持)」。**mock-audit.ts** (13 events): CASE-2026-0142 (Demo Chapter 1、2 events) + CASE-2026-0118 (Demo Chapter 2 narrative 中核、9 events incl. 適用範囲 2 Alert) + CASE-2026-0148 (UC-BO-02 Dashboard 要注意、2 events)、Demo Chapter 2 全 flow trace (PDF 受付 → AI 入力 → 入力者差戻し → AI 日次分析 → 手順承認 → 関連ルール更新 Alert → 入力者再確認 → 承認者承認 → 反映)。**CR R44 governance term paraphrase 規範 pre-flight 適用**: review / trigger / Forced Downgrade / upgrade / precision / layout は mock data summary / actorLabel / EVENT_TYPE_LABEL から完全排除、internal SSOT 参照 (snake_case schema field 名) は JSDoc + UI detail panel label として keep。**国際送金** (restricted boundary pack) は `workflow_id` 含めず、`mock-audit.ts` 対象外 (DOC-OV-00 §2.1)
